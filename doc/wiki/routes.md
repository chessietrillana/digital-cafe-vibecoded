...
| `/` | `cafe.views.home` | **No** | Lists active products. Greets the customer by first name (or username) when logged in; shows a generic "Welcome to Chessie's Digital Cafe!" heading when not. Nav shows Cart/Order History/Log out when authenticated, or Log in/Sign up links when not. |
